...
int main(int argc, char** argv)
{
    ...
    if (strcmp(argv[1], "play1") == 0) {
        ...
        filefd1 = Ql_PcmFileOpen(argv[2], &mediaParas, NULL);
	...
	// Command: <process> play2 <audio file1> <audio file2>
        ...
        mediaParas.format = AUD_STREAM_FORMAT_PCM;
        filefd1 = Ql_PcmFileOpen(argv[2], &mediaParas, NULL);
        filefd2 = Ql_PcmFileOpen(argv[3], &mediaParas, NULL);
    ...
    else if (strcmp(argv[1], "recd1") == 0) {//record and playback
        if (argc != 2) {
    ...
    else if (strcmp(argv[1], "recd2") == 0) {//record and playback and save
        if (argc != 3) {
			quec_log("< recorder needs more parameter\n\r");
            return 0;
        }
    ...
    else if (strcmp(argv[1], "tone") == 0) {//play tone
        if (argc == 2) {
