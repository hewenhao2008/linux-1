...
#if 1//ndef QUECTEL_MODEM_RESTORE_UBI // no define QUECTEL_MODEM_RESTORE_UBI
...
out:
	if (!quiet) {
		...
		printk("@Ramos UBI Error  6661111 ubifs_check_node vi.dev_num=%d, vi.vol_id=%d , \r\n",  (c)->vi.ubi_num,(c)->vi.vol_id);
			...
			if(2 == c->vi.ubi_num) // usrdata partition	attach on ubi2 in normal mode
			{
		...
		if( 1 == c->vi.ubi_num)
		{
			...
			Quectel_Set_Partition_RestoreFlag("modem",6);
		}
		else
		{  dump_stack(); }
#else
		...
		//ռ˳Linuxsystemʱ ubi 0 һ£modem ubi سҲ
			printk("@Ramos set restore systemfs flag here 6661111  vi.dev_num=%d, vi.vol_id=%d , \r\n",  (c)->vi.ubi_num,(c)->vi.vol_id);
			Quectel_Set_Partition_RestoreFlag("system",6);
		}
#endif		
	}
	return err;
...
out:
	...
	{
	//ռ˳Linuxsystemʱ ubi 0 һ£modem ubi سҲ
		printk("@Ramos set restore systemfs flag here 6662222 vi.dev_num=%d, vi.vol_id=%d , \r\n", (c)->vi.ubi_num,(c)->vi.vol_id);
		Quectel_Set_Partition_RestoreFlag("system",6);
	}
#endif	
	return -EINVAL;
...
out:
	...
	if (!c->probing) {
		...
		{
	//ռ˳Linuxsystemʱ ubi 0 һ£modem ubi سҲ
			printk("@Ramos set restore systemfs flag here 6663333vi.dev_num=%d, vi.vol_id=%d , \r\n", (c)->vi.ubi_num,(c)->vi.vol_id);
			Quectel_Set_Partition_RestoreFlag("system",6);
		}
#endif	
	}
