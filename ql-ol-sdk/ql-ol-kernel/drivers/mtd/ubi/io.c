	...
	if (err) {
		...
		if (mtd_is_bitflip(err)) {
			...
			ubi_msg(ubi, "fixable bit-flip detected at PEB %d",
				pnum);
#if 1 // def  QUECTEL_MODEM_BACKUP	  //Ramos 20170511 add for ubi read err 
			if(len != read)
			{
				printk("@Ramos UBI Error 999999 set restore UBI =%d,	\r\n", ubi->ubi_num);
			#if 1   // quectel add usrdata partition erase format
				if(2 == ubi->ubi_num) // usrdata partition  attach on ubi2 in normal mode
				{
					Quectel_Erase_Partition("usr_data");
				}
			#endif
				if(1 == ubi->ubi_num)
				{
					Quectel_Set_Partition_RestoreFlag("modem",9);
				}
				
				if( 0 == ubi->ubi_num)
				{
				//°´ÕÕ¼ÓÔØË³Ðò£¬LinuxµÄsystem¼ÓÔØÊ±ÊÇ ubi 0£¬ ËùÒÔÕâÀïÏÞÖÆÒ»ÏÂ£¬±ÜÃâmodem ubi ¼ÓÔØ³öÏÖÕâ¸ö´íÎóÒ²»á½øÈëÕâÀï¡£
					printk("@Ramos set restore systemfs flag here 99999\r\n");
					Quectel_Set_Partition_RestoreFlag("system",9);
				}
		...
		if(1 == ubi->ubi_num)
		{
		//	Quectel_Erase_Partition("modem");
			Quectel_Set_Partition_RestoreFlag("modem",5);
		}
#endif
			...
			if( 0 == ubi->ubi_num)
			{
			//ռ˳Linuxsystemʱ ubi 0 һ£modem ubi سҲ
				printk("@Ramos set restore systemfs flag here 55555\r\n");
				Quectel_Set_Partition_RestoreFlag("system",5);
			}
#endif
			ubi_assert(0);
			err = -EIO;
...
#if 1 // def  QUECTEL_MODEM_BACKUP   //  add for modem file backup
		if(len != read)
		{
			...
			{
			//°´ÕÕ¼ÓÔØË³Ðò£¬LinuxµÄsystem¼ÓÔØÊ±ÊÇ ubi 0£¬ ËùÒÔÕâÀïÏÞÖÆÒ»ÏÂ£¬±ÜÃâmodem ubi ¼ÓÔØ³öÏÖÕâ¸ö´íÎóÒ²»á½øÈëÕâÀï¡£
				printk("@Ramos set restore systemfs flag here 55555222\r\n");
				Quectel_Set_Partition_RestoreFlag("system",5);
			}
		}
#endif
